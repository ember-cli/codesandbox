  ...
  processString(ctx, contents, relativePath, forceInvalidation, instrumentation) {
    ...
    return cache.get(key).then(entry => {
      if (entry.isCached && !forceInvalidation) {
        fs.appendFileSync('/tmp/sandbox/cache-keys.txt', `::hit::${relativePath}::${contents.length}::${hash}::${key}\n`);
        ...
        fs.appendFileSync('/tmp/sandbox/cache-keys.txt', `::miss::${relativePath}::${contents.length}::${hash}::${key}\n`);
