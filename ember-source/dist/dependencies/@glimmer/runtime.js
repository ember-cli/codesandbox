import { assign, symbol, Stack, fillNulls, initializeGuid, _WeakSet, isObject, dict, emptyArray, EMPTY_STRING_ARRAY, debugToString, decodeHandle, isNonPrimitiveHandle, isErrHandle, unwrapTemplate, unwrapHandle, clearElement, constants, decodeImmediate, encodeHandle, encodeImmediate, isHandle, isSmallInt, buildUntouchableThis, isDict } from '@glimmer/util';
...
import { warnIfStyleNotTrusted, toBool, assertGlobalContextWasSet, getPath, setPath } from '@glimmer/global-context';
...
function resolveHelper(constants$$1, definition, ref) {
  let handle = constants$$1.helper(definition, null, true);
...
, vm => {
  ...
  let instanceRef = createComputeRef(() => {
    ...
    let handle = constants$$1.modifier(hostDefinition, null, true);
    ...
    let definition = constants$$1.getValue(handle);
    let {
      manager
    } = definition;
...
, (vm, {
  ...
  let constants$$1 = vm[CONSTANTS];
  let owner = vm.getOwner();
  let isStrict = constants$$1.getValue(_isStrict);
  ...
  if (typeof component === 'string') {
    ...
    let resolvedDefinition = resolveComponent(vm.runtime.resolver, constants$$1, component, owner);
    definition = resolvedDefinition;
...
, (vm, {
  ...
  let evalInfo = constants$$1.getArray(decodeHandle(_evalInfo));
